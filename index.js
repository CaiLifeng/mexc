...
function createTrade(symbol) {
    return async function trade(){
        try {
            await tradeRequest.forward({
                url:'https://futures.mexc.com/api/v1/private/order/submit', 
                body:{"symbol":symbol,"side":1,"openType":2,"type":"5","vol":1,"leverage":20,"priceProtect":"0"}, 
                method:'POST'
            })
            await tradeRequest.forward({
                url:'https://futures.mexc.com/api/v1/private/order/submit', 
                body:{"symbol":symbol,"side":4,"openType":2,"leverage":20,"type":5,"vol":1,"priceProtect":"0"}, 
                method:'POST'
            })
            logger.info(`完成一次${symbol}买卖`);
        } catch(error) {
            logger.error(error);
        }
...
const SHIB_USDT_TRADE = createTrade('SHIB_USDT'); // SHIB_USDT交易手续费0.0000 USDT
// const SHIT_USDT_TRADE = createTrade('SHIT_USDT'); // SHIT_USDT交易手续费0.0001 USDT
SHIB_USDT_TRADE();
// SHIT_USDT_TRADE();
